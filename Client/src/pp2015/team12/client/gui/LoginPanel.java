...
public class LoginPanel extends JLayeredPane implements ActionListener{
	...
	private boolean checkLogin(String user, String pass){
		// login request
		controller.sendMessage(new LoginRequestMsg(2,user,pass));
	...
	public Boolean checkReponse(Message message){
		if(((LoginReplyMsg)message).isSuccess()){
			if(rememberPassword.isSelected()){
				BufferedWriter writer = null;
				try {
				    writer = new BufferedWriter( new FileWriter("credentials.txt"));
				    writer.write(usernameTf.getText());
				    writer.newLine();
				    writer.write(new String(passwordTf.getPassword()));
				    writer.close();
				}
				catch ( IOException e){
					System.out.println("Error saving credentials");
