...
public class DataBaseController
{
	 ...
	 * @author Staufenberg, Thomas, 5820359
	 * */
	protected static DataBaseController getInstance()
	{
	 ...
	 * inserts some test user</br>
	 * if the database has been initialized before, this method changes nothing
	 * @author Staufenberg, Thomas, 5820359
	 * */
	private void initializeDatabaseSchema()
	{
		System.out.println("DataBaseController: Initialisiere Datenbankschema.");
		...
		try(Statement stmt = this.dbConnection.createStatement())
		{
			...
			System.out.println("Testbenutzer werden erzeugt.\n");
			Random randomGenerator = new Random();
			for(int i = 1; i < 10; i++)
			{
				if(this.createNewUser(("benutzer" + i ), "passWord"))
				{
					PlayerCharacter currentCharacter = new Mage(1);
					currentCharacter.getStatistics().updateDeathCount(randomGenerator.nextInt(1857));
					currentCharacter.getStatistics().updateMonsterKillCount(randomGenerator.nextInt(2935));
					currentCharacter.getStatistics().updateGoldEarned(randomGenerator.nextInt(98431));
					currentCharacter.getStatistics().updateTimePlayed(randomGenerator.nextInt(4923));
					currentCharacter.setUsername(("benutzer" + i));
					this.linkCharacterToUser(currentCharacter);
				}
				else
		...
		{
			System.err.println("DataBaseController: Fehler beim Initialisieren der Datenbank!");
			//TODO handle exception
		}
		finally
		{
	...
	{
		System.out.println("DataBaseController: Beginne Ladesequenz fuer Benutzer: " + paramUsername);
		this.establishConnection();
		...
		try(Statement retrieveIDStmt = this.dbConnection.createStatement(); ResultSet retrieveIDResult = retrieveIDStmt.executeQuery("SELECT playerID, inventoryID, saveGameID, statisticsID FROM registereduser JOIN playerallocation ON registereduser.username = playerallocation.username WHERE playerallocation.username = '" + paramUsername + "'"))
		{
				...
				Statement itemStmt = this.dbConnection.createStatement(); ResultSet itemResult = itemStmt.executeQuery("SELECT alloc.slotID, item.itemID, equipSlot, attackValue, defenseValue, hpValue, levelRestriction, itemPrice, armorPartsRevenue FROM (SELECT slotID, itemID FROM inventoryitemallocation WHERE inventoryID = " + retrieveIDResult.getInt(2) + ") AS alloc JOIN item ON alloc.itemID = item.itemID ORDER BY alloc.slotID"))
			{
				...
				currentPlayer.setUsername(paramUsername);
				System.out.println("DataBaseController: Ladesequenz erfolgreich abgeschlossen!");
				ServerEngine_III_Utility.printPlayerInformation("DataBaseController", currentPlayer);
	...
	protected Object[][] loadHighscore(String paramFilterAttribute)
	{
		...
		{
			System.err.println("DataBaseController: Fehler beim Auslesen von Highscoredaten!");
			//TODO handle exception
			return null;
		}
		finally
	...
	private boolean isValidUsername(String paramUsername)
	{
		...
		{
			System.err.println("DataBaseController: Fehler beim Pruefen des Benutzernamens!");
			//TODO handle exception
		}
		return false;
	}
	...
	{
		System.out.println("DataBaseController: Speichere ausgewaehlten Charakter in der Datenbank.");
		String currentUsername = paramPlayer.getUsername();
		...
		try (Statement stmt = this.dbConnection.createStatement(); ResultSet consitencyCheckResult = stmt.executeQuery("SELECT playerID FROM playerallocation WHERE username = '" + currentUsername + "'"))
		{
			...
			if (consitencyCheckResult.getInt(1) == 1)
			{
				...
				this.dbConnection.commit();							// end transaction
				System.out.println("DataBaseController: Charakter erfolgreich gespeichert und mit Benutzer verbunden.");
				return true;
			}
		}
		catch (SQLException sqlE)
		{
			...
			try
			{
				System.err.println("DataBaseController: Fehler beim Speichern von Charakter: attempt dbConnection.rollback()");
				this.dbConnection.rollback(); 						// onException: attempt rollback
			}
			catch (SQLException embSqlE)
			{
				System.err.println("DataBaseController: Fehler beim Speichern von Charakter: dbConntection.rollback() -> failed");
				// TODO return error: rollback failed
			}
			System.err.println("DataBaseController: Fehler beim Speichern von Charakter!");
			// TODO handle exception
		}
		finally
		{
			...
			{
				System.err.println("DataBaseController: Fehler beim Speichern von Charakter: dbConnection.setAutocommit(true) -> failed");
				//TODO handle exception: setAutocommit(true) failed -> data integrity cannot be guaranteed
			}
		}
		return false;
	...
	/**
	 * resets the database to its default values by deleting all unneeded data</br>
	 * and restoring all AUTO_INCREMENT values to their previous settings
	 * @author Staufenberg, Thomas, 5820359
	 * */
	protected void clearDB()
	{
		System.out.println("DataBaseController: Bereinige Datenbank.");
		this.establishConnection();
		
		try(Statement stmt = this.dbConnection.createStatement())
		{
			stmt.addBatch("DELETE FROM playerallocation");
			stmt.addBatch("DELETE FROM inventoryitemallocation");
			stmt.addBatch("DELETE FROM inventory");
			stmt.addBatch("DELETE FROM item WHERE itemID > 3");
			stmt.addBatch("DELETE FROM player WHERE playerID > 1");
			stmt.addBatch("DELETE FROM registereduser");
			stmt.addBatch("DELETE FROM savegame");
			stmt.addBatch("DELETE FROM statistics");
			stmt.addBatch("ALTER TABLE inventory AUTO_INCREMENT = 1");
			stmt.addBatch("ALTER TABLE inventoryitemallocation AUTO_INCREMENT = 1");
			stmt.addBatch("ALTER TABLE player AUTO_INCREMENT = 2");
			stmt.addBatch("ALTER TABLE playerallocation AUTO_INCREMENT = 1");
			stmt.addBatch("ALTER TABLE savegame AUTO_INCREMENT = 1");
			stmt.addBatch("ALTER TABLE statistics AUTO_INCREMENT = 1");
			stmt.addBatch("ALTER TABLE item AUTO_INCREMENT = 3");
			
			stmt.executeBatch();
			
			System.out.println("DataBaseController: Bereinigung erfolgreich.");
			System.out.println("----------------------------------------------------------------------------------");
			System.out.println("----------------------------------------------------------------------------------");
			System.err.println("-------------------Sie koennen die Testumgebung nun neu starten-------------------");
			System.out.println("----------------------------------------------------------------------------------");
			System.out.println("----------------------------------------------------------------------------------");
		}
		catch(SQLException sqlE)
		{
			System.err.println("DataBaseController: Fehler bei Bereinigung der Datenbank!");
			//TODO handle exception
		}
		finally
		{
