...
import pp2015.team12.shared.message.CharacterListMsg;
...
public class ClientCommunication {
		...
		/**
		 * temporally saves every received message in msgList
		 * @author Heusser,Caspar
		 * @param message
		 */
		...
		public void handleMessage(){
			...
			while(this.msgList.get(0)!=null){
				...
				if (message instanceof MoveMsg){
					System.out.println("Bearbeite MoveMsg");
				...
				if (message instanceof EngageFightMsg){
					...
					this.cE.engageFight((EngageFightMsg)message);				//noch in TestEngine
				}
				...
				if(message instanceof SalvageRequestMsg){
					System.out.println("Message an server"); //server braucht id + player
				}
				if(message instanceof LocInvMsg){
					this.cE.updateInventory((LocInvMsg)message);
				}
				if(message instanceof LevelChangeReqMsg){
					this.cE.useDoor((LevelChangeReqMsg)message);
				}
				if(message instanceof RegistrationReqMsg){
					System.out.println("Weiterleiten an server");
				}
				...
				if(message instanceof ShowAvailableCharacterMsg ){
					System.out.println("Weiterleiten an gui");
				}
